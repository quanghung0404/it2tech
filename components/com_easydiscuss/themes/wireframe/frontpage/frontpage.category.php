...
<div class="discuss-category-header">
	<div class="discuss-subscribe pull-right">
		...
		<?php if($this->config->get('main_ed_categorysubscription')){ ?>
			<?php if(!$category->container) { ?>
				<?php echo DiscussHelper::getSubscriptionHTML( $this->my->id , $category->id, 'category'); ?>
			<?php } ?>
		<?php } ?>
	...
	<div class="discuss-category-info">
		...
		<div class="fd-cf mb-10 discuss-category-statistic">
			<?php if( $this->config->get( 'layout_category_stats' ) ){ ?>
			...
			<?php if( $this->config->get( 'layout_show_moderators' ) && $moderatorHTML !== false && !empty( $moderatorHTML ) ) { ?>
			<span>
