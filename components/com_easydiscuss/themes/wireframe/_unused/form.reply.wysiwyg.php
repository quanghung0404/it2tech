...
<div class="discuss-composer <?php echo $composer->classname; ?> discuss-composer-<?php echo $composer->operation; ?>"
	...
	<div class="o-alert o-alert--info replyNotification" style="display: none;"></div>
		...
		<div class="discuss-content">
			<form name="dc_submit" autocomplete="off" class="form-horizontal" action="<?php echo DiscussRouter::_('index.php?option=com_easydiscuss&controller=posts&task=saveReply'); ?>" method="post">
				...
				<div class="discuss-form">
					...
					<?php if ($this->config->get('main_location_reply')) { ?>
					...
					<?php } else if (ED::captcha()->showCaptcha()){ ?>
						<?php echo ED::captcha()->getHTML();?>
					<?php } ?>

					<div class="fd-cf">
						<div class="pull-left">
							<a href="<?php echo EDR::_('index.php?option=com_easydiscuss&view=post&id=' . $post->parent_id);?>" class="btn btn-medium btn-danger"><?php echo JText::_('COM_EASYDISCUSS_BUTTON_CANCEL'); ?></a>
						</div>
					...
					<input type="hidden" name="title" value="Re: <?php echo ED::string()->escape($parent->title); ?>" />
					...
					<input type="hidden" name="user_type" value="<?php echo $this->my->id == 0 ? 'guest' : ''; ?>" />
				</div>
			</form>
		</div>
	</div>
