...
<span class="kmt-location"<?php if( $system->konfig->get( 'enable_schema' ) ) echo ' itemprop="contentLocation" itemscope itemtype="http://schema.org/Place"'; ?>>
	<?php if( $row->address && $row->latitude && $row->longitude ) { ?>
		...
		<i></i><a href="http://maps.google.com/maps?z=15&amp;q=<?php echo $this->escape( $row->latitude );?>,<?php echo $this->escape( $row->longitude );?>" target="_blank"><?php echo $this->escape( $row->address );?></a>
	<?php } else { ?>
		<?php if( $row->address ) { ?>
