 ...
 */
var tooltips_timeout=tooltips_timeout||0;var tooltips_delay_hide=tooltips_delay_hide||0;var delay_hide_touchscreen=delay_hide_touchscreen||0;var tooltips_use_auto_positioning=tooltips_use_auto_positioning||0;var tooltips_fallback_position=tooltips_fallback_position||'bottom';/* <<< [PRO] <<< */(function($){"use strict";$(document).ready(function(){var tt_timeout=null;var tt_timeoutOff=0;$('.nn_tooltips-link.hover').popover({trigger:'hover',container:'body',delay:{show:5,hide:tooltips_delay_hide}});$('.nn_tooltips-link.click').popover({trigger:'manual',container:'body'}).click(function(evt){tooltipsShow($(this),evt,'click');}).mouseout(function(evt){tooltipsSetTimer($(this),tooltips_timeout);});$('.nn_tooltips-link.sticky').popover({trigger:'manual',container:'body'}).mouseover(function(evt){tooltipsShow($(this),evt,'sticky');}).mouseout(function(evt){tooltipsSetTimer($(this),tooltips_timeout);});$('html').click(function(){$('.nn_tooltips-link').popover('hide');});$('html').one('touchstart',function(){$('.nn_tooltips-link.hover').popover({trigger:'manual',container:'body'}).click(function(evt){tooltipsShow($(this),evt,'click');tooltipsSetTimer($(this),delay_hide_touchscreen);});});$('html').on('touchstart',function(e){if($(e.target).closest('.nn_tooltips').length){return;}
$('.nn_tooltips-link').popover('hide');});$('.nn_tooltips-link').on('touchstart',function(evt){evt.stopPropagation();});function tooltipsShow(el,evt,cls){evt.stopPropagation();clearTimeout(tt_timeout);$('.nn_tooltips-link.'+cls).each(function(){if($(this).data('popover')!=el.data('popover')){$(this).popover('hide');}});if(!el.data('popover').tip().hasClass('in')){el.popover('show');}
...
var $tip=this.tip();$tip.mouseover(function(evt){$tip.addClass('hover');}).mouseout(function(evt){$tip.removeClass('hover');});this.setContent();if(this.options.animation){$tip.addClass('fade');}
