	...
	nnTabsPopup = {
		...
		fillFromSelection: function() {
			...
			$.each(params_sets, function(i, params) {
				...
				$.each(self.selects, function(s, select) {
					...
					self.setSelectOption(id + '[' + select + ']', params[select]);
				});
			});
		},
		...
		getParamsFromSelection: function(selection) {
			...
			for (var i = 1; i < count; i++) {
				...
				$.each(this.selects, function(s, select) {
					var regex = new RegExp('\\|' + select + '=(.*?)(\\||$)');
					var selected_name = title.match(regex);

					if (!selected_name) {
						return;
					}

					selected_name = selected_name[1].trim();

					$('select[name="tab_1[' + select + ']"] option').each(function($i, el) {
						var option_name = $(el).html().replace(/^[\s-]*/, '').trim();

						if (option_name.toLowerCase() !== selected_name.toLowerCase()) {
							return;
						}

						params[select] = $(el).attr('value');
					});
		...
		insertText: function() {
			...
			$('.tab-pane').each(function($i, el) {
				...
				$.each(self.selects, function(s, select) {
					...
					var option_name = option_field.html().replace(/^[\s-]*/, '').trim();

					title += '|' + select + '=' + option_name;
