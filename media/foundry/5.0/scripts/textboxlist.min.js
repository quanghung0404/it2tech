...
textField.data("realEnterKey",keyCode==KEYCODE.ENTER)},"{textField} keypress":function(textField,event){var keydownIsEnter=textField.data("realEnterKey"),keypressIsEnter=event.keyCode==KEYCODE.ENTER;textField.data("realEnterKey",keydownIsEnter&&keypressIsEnter);var keyword=$.trim(self.textField().val());switch(event.keyCode){case KEYCODE.ENTER:if(textField.data("realEnterKey")){var event=self.trigger("useItem",[keyword]),item=event.item;if(void 0===item&&(item=keyword),item===!1||null===item)return;var item=item.split(",");
$.each(item,function(i,item){self.addItem(item)}),textField.val("")}}},"{textField} keyup":function(textField,event){var item=$.trim(self.textField().val()),canRemoveItemUsingBackspace="canRemoveItemUsingBackspace";switch(event.keyCode){case KEYCODE.BACKSPACE:if(""===item)if(self[canRemoveItemUsingBackspace]){var prevItem=textField.prev(self.item.selector);if(prevItem.length>0){var id=prevItem.data("id"),item=self.items[id];!item.locked&&self.removeItem(id)}}else self[canRemoveItemUsingBackspace]=!0;break;default:self[canRemoveItemUsingBackspace]=!1
