...
class FPSSModelSlides extends FPSSModel
{
	...
	function import()
	{
		...
		$categoryModel = FPSSModel::getInstance('category', 'FPSSModel');
		...
		if (version_compare(JVERSION, '1.6.0', 'ge'))
		{
			...
			$categoryParams = new JRegistry('');
			foreach ($xml->params as $paramGroup)
			{
				foreach ($paramGroup->param as $param)
				{
					if ((string)$param->attributes()->type != 'spacer')
					{
			...
			foreach ($xml->params as $paramGroup)
			{
				foreach ($paramGroup->param as $param)
				{
					if ((string)$param->attributes()->type != 'spacer')
					{
						$slideParams->set((string)$param->attributes()->name, (string)$param->attributes()->default);
					}
				}
			}
		...
		foreach ($xml as $category)
		{
			...
			foreach ($slides as $slide)
			{
				...
				$images = array();
				foreach ($slide->children() as $key => $child)
				{
					if (version_compare(JVERSION, '1.6.0', 'ge'))
					{
						if ($key != 'image')
