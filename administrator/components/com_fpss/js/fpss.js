/**
 * @version		$Id: fpss.js 2186 2012-11-15 19:22:34Z joomlaworks $
...
$FPSS(document).ready(function(){
	...
	if($FPSS('.slideForm').length || $FPSS('.categoryForm').length){
		$FPSS('.fpssBackendEditPageContainer').find('select, input, button, textarea').uniform();
	}

	// Common functions
	$FPSS('#jToggler').click(function(){
		if($FPSS(this).attr('checked')){
			$FPSS('input[id^=cb]').attr('checked', true);
			$FPSS('input[name=boxchecked]').val($FPSS('input[id^=cb]:checked').length);
		} else {
			$FPSS('input[id^=cb]').attr('checked', false);
	...
	switch(view){
		case 'slide':
			...
			*/
			...
			$FPSS('a[href=#fpssSlideAdvancedTab], a[href=#fpssSlidePermissionsTab]').click(function(){
