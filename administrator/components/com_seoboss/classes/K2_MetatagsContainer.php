       ...
       $db = JFactory::getDBO();
        ...
        foreach($items as $item){
            if($item->title != ''){
                ...
                $sql = "UPDATE #__k2_items SET metakey=".$db->quote($item->title)." WHERE id=".$db->quote($item->item_id);
                $db->setQuery($sql);
                $db->query();
            }
        }
        ...
        foreach($ids as $key=>$value){
            if(!is_numeric($value)){
                ...
                if($title){
                  ...
                  $sql = "UPDATE #__k2_items SET metakey=".$db->quote($title)." WHERE id=".$db->quote($value);
                  $db->setQuery($sql);
                  $db->query();
                }
            }
