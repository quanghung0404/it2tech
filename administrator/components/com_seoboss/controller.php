...
class SeobossController extends JBController
{
    ...
    private function metatags_manager( $task ){
        ...
        $view->assign('filter', $metatagsContainer->getFilter());
        $view->assign('availableTypes', MetatagsContainerFactory::getFeatures() );
        $view->assignRef('pageNav', $pageNav);
        $view->assign('order', JRequest::getCmd("filter_order", "title"));
        $view->assign('order_Dir', JRequest::getCmd("filter_order_Dir", "ASC"));
        $view->display();
    }
    ...
    function duplicated_save(){
      $this->setRedirect("index.php?option=com_seoboss&task=duplicated_pages", "This feature is available in PRO version only.");
    }
