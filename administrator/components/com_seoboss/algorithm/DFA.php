  ...
  private function performReplace($content){
      ...
      $keywordsToReplace = array();
          ...
          foreach($this->keywords as $keywordData){
    	   ...
    	   while(( $currentPosition = $this->encoding?
             @mb_strpos($upper, $upperKeyword, $currentPosition, $this->encoding):
      ...
      foreach($searchPositions as $currentPosition){
         ...
         (mb_substr($content, 0, $currentPosition[0],$this->encoding).
           '<'.
           $this->hilight_tag.
           (empty($this->hilight_class)?"":' class="'.$this->hilight_class.'"').
           '>'.
           mb_substr($content, $currentPosition[0], $currentPosition[1] - $currentPosition[0], $this->encoding).
           '</'.$this->hilight_tag.'>'.
        	 mb_substr($content, $currentPosition[1], 123456, $this->encoding )):
      	 (substr($content, 0, $currentPosition[0]).
           '<'.
           $this->hilight_tag.
           (empty($this->hilight_class)?"":' class="'.$this->hilight_class.'"').
           '>'.
           substr($content, $currentPosition[0], $currentPosition[1] - $currentPosition[0]).
           '</'.$this->hilight_tag.'>'.
